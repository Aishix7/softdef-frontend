...
export const SideBar = ({ onColorSelect }) => {
  ...
  const [isVisible, setIsVisible] = useState(false); // For mobile toggle
    ...
    <>
      ...
      <div className="sm:hidden flex justify-end mb-2">
        <button
          ...
          className="bg-[#40bfff] text-white font-medium px-4 py-2 rounded"
        >
          {isVisible ? "Close Filters" : "Filter"}
        </button>
      </div>

      {/* Sidebar */}
      <div
        className={`flex flex-col bg-white space-y-4 p-2 sm:block ${
          isVisible
            ? "block absolute z-50 top-14 left-0 w-full sm:w-auto"
            : "hidden"
        }`}
      >
        ...
        <div className="bg-[#f6f7f8] rounded-md p-7">
          <div
            className="font-medium cursor-pointer pb-4"
            onClick={toggleBrand}
          >
            BRAND {brandOpen ? "▲" : "▼"}
          </div>
